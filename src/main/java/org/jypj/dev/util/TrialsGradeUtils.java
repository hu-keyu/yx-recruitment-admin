...
public class TrialsGradeUtils {
	 ...
	 * @return
	 */
	private static Map<String ,List<ScoreGradeTrial>> groupByPositionId(List<ScoreGradeTrial> grades){
	 ...
	 * @return
	 */
	private static Map<String, List<ScoreGradeTrial>> isPassScore(List<ScoreGradeTrial> grades,int interviewEnterLine){
		Map<String ,List<ScoreGradeTrial>> mapDates=new HashMap<String ,List<ScoreGradeTrial>>();
	 ...
	 * @return
	 */
	private static Map<String, List<ScoreGradeTrial>> isPassRatio(List<ScoreGradeTrial> grades,int interviewEnterPropo){
		...
		if(index > 0 && index <= totleCount){
			...
			if(flag){//有相同的分数
				ScoreGradeTrial grade=grades.get(index-1);
				Map<String, List<ScoreGradeTrial>> datas=isPassRatio(ratioPass,ratioNoPass,Double.valueOf(grade.getGrade()).doubleValue());
	 ...
	 * @return
	 */
	private static Map<String, List<ScoreGradeTrial>> isPassRatio(List<ScoreGradeTrial> ratioPass, List<ScoreGradeTrial> ratioNoPass, double score) {
		Map<String ,List<ScoreGradeTrial>> mapDates=new HashMap<String ,List<ScoreGradeTrial>>();
		List<ScoreGradeTrial> sameGrade=new ArrayList<ScoreGradeTrial>();
		for(ScoreGradeTrial grade : ratioNoPass){
	 ...
	 * @return
	 */
	private static List<ScoreGradeTrial> orderByScore(List<ScoreGradeTrial> grades){
	 ...
	 * @return
	 */
	private static boolean hasRepeatGrade(List<ScoreGradeTrial> grades){
		boolean flag=false;
		ScoreGradeTrial [] gradeArray=grades.toArray(new ScoreGradeTrial []{});
	 ...
	 * @return
	 */
	private static Map<String, List<ScoreGradeTrial>> isEmploy(List<ScoreGradeTrial> grades,int enterCondition,int interviewEnterPropo,int interviewEnterLine){
		Map<String ,List<ScoreGradeTrial>> mapDates=new HashMap<String ,List<ScoreGradeTrial>>();
		if(Enter_Condition_Score_Ratio == enterCondition){//比例分数线 
			mapDates=isPassScore(grades, interviewEnterLine);
			List<ScoreGradeTrial> gradepass=mapDates.get("pass");
			List<ScoreGradeTrial> gradeNoPass=mapDates.get("noPass");
	 ...
	 * @return Map
	 */
	public static Map<String ,List<ScoreGradeTrial>> groupByPositionIdIsEmploy(List<ScoreGradeTrial> grades,int enterCondition,int interviewEnterPropo,int interviewEnterLine){
		Map<String ,List<ScoreGradeTrial>> datas=groupByPositionId(grades);
		List<ScoreGradeTrial> gradePass=new ArrayList<ScoreGradeTrial>();
		List<ScoreGradeTrial> gradeNoPass=new ArrayList<ScoreGradeTrial>();
		for(Entry<String, List<ScoreGradeTrial>> items : datas.entrySet()){
			List<ScoreGradeTrial> gradeItems=items.getValue();
			Map<String ,List<ScoreGradeTrial>> data=isEmploy(gradeItems,enterCondition,interviewEnterPropo,interviewEnterLine);
